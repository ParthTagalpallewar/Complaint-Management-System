...
$city=$_POST['city'];
$rollno=$_POST['rollno'];
$query=mysqli_query($con,"update users set fullName='$fname',contactNo='$contactno',address='$address',department='$department',city='$city',rollno='$rollno' where userEmail='".$_SESSION['login']."'");
if($query)
{
...
<option value="<?php echo htmlentities($row['department']);?>"><?php echo htmlentities($st=$row['department']);?></option>
<?php $sql=mysqli_query($con,"select departmentName from department ");
while ($rw=mysqli_fetch_array($sql)) {
  if($rw['departmentName']==$st)
  {
    continue;
  }
  else
  {
  ?>
  <option value="<?php echo htmlentities($rw['departmentName']);?>"><?php echo htmlentities($rw['departmentName']);?></option>
<?php
}}
?>
...
<div class="col-sm-4">
<input type="text" name="city" required="required" value="<?php echo htmlentities($row['city']);?>" class="form-control">
</div>
</div>
...
<div class="col-sm-4">
<input type="text" name="rollno" maxlength="7" required="required" value="<?php echo htmlentities($row['rollno']);?>" class="form-control">
</div>
